...
@Component({
  selector: 'app-a-evaluationfactors',
  templateUrl: './a-evaluationfactors.component.html',
  styleUrls: ['./a-evaluationfactors.component.css']
})
export class AEvaluationfactorsComponent implements OnInit {
  ...
  console.log(e)
    ...
    let change = `A <b>VOLUME</b> has been added in <b>EVALUTATION FACTORS</b>. `,data4
    this.changes.push(change);
    data4={
      evaluationChanges:this.changes
    }
  ...
  addFactors(j, event) {
    ...
    setTimeout(() => {
     ...
     let change = `Factor <b>${newVal}</b> has been added to volume <b>${this.evaluations.get("evaluationCriteria").controls[j].value.volumeName.toUpperCase()} </b> in <b>EVALUTATION FACTORS</b>. `,data4
    this.changes.push(change);
    data4={
      evaluationChanges:this.changes
    }
  ...
  removeFactors(control, index) {
    control.factorRows.removeAt(index);
    this.onChange(this.evaluations);
    let change = `Factor <b>${control.factorRows.value.factorName}</b> has been removed from volume <b>${control.volumeName.value.toUpperCase()} </b> in <b>EVALUTATION FACTORS</b>. `,data4
    this.changes.push(change);
    data4={
      evaluationChanges:this.changes
    }
  ...
  removeVolumes(i) {
    const control = <FormArray>this.evaluations.get("evaluationCriteria");
    let change = `Volume <b>${control.value[i].volumeName.toUpperCase()} </b> has been deleted in <b>EVALUTATION FACTORS</b>. `,data4
    this.changes.push(change);
    data4={
      evaluationChanges:this.changes
    }
  ...
  getAp() {
    this.ap.getsoldappids(this.solno).subscribe(response => {
      ...
      this.evaluationsId = amres.evaluationCriteriaId;
     ...
     this.ap.getSolicitation(response.solId).subscribe(res =>{
      ...
      this.ap.getUserprofile().subscribe(response => {
        ...
        let validate =(text)=>{
         ...
         this.status == "Initiated" ||  this.status !='Re-Published'?validate("true"):validate("false");  
      })
      ...
      .subscribe((response5) => {
        ...
        console.warn(typeof response5.evaluationChanges)
        if( response5.evaluationChanges == undefined){
         this.changes = [];
        }else{
          this.changes = response5.evaluationChanges
        }
        ...
        .subscribe(response => {
          ...
          if (response) {
            this.evaluations
              ...
              this.orginaldata = response.mandatoryCriterias
              this.evaluations
  ...
  mandatoryCriteria(event) {
    ...
    // if(evaluations.value.mandatoryCriterias != this.orginaldata){
  ...
  MCC(){
   ...
   let st= this.evaluations
            .get("mandatoryCriterias").value

              if(st != this.orginaldata){
                let change = `<b>MANDATORY CRITERIA</b> was updated.`,data4
                this.changes.push(change);
                data4={
                  evaluationChanges:this.changes
                }
  ...
  orginalfactortext
  SetOrginalFactorText(index,Volume){
    this.orginalfactortext = Volume.value.factorRows[index].factorText
    
  }

  FactorChangeTextChange(index,Volume){
    if(Volume.value.factorRows[index].factorText != this.orginalfactortext){
      let change = `Factor <b>${Volume.value.factorRows[index].factorName}</b> has been updated in Volume <b>${Volume.value.volumeName.toUpperCase()} </b> in <b>EVALUTATION FACTORS</b> `,data4
      this.changes.push(change);
      data4={
        evaluationChanges:this.changes
      }
