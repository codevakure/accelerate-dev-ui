...
@Component({
  selector: 'app-a-specialcontractreq',
  templateUrl: './a-specialcontractreq.component.html',
  styleUrls: ['./a-specialcontractreq.component.css']
})
export class ASpecialcontractreqComponent implements OnInit {
    ...
    if(event.html != undefined){
      ...
      if(event.source != 'api' &&  event.html == undefined){
        if(this.data != this.orginaldata){
          ...
          data4={
            specialcontractChanges:this.changes
          }
  ...
  getSectiondata() {
    ...
    this.ap.getSolicitation(response.solId).subscribe(response => {
      ...
      .subscribe((response5) => {
        ...
        console.warn(typeof response5.specialcontractChanges)
        if( response5.specialcontractChanges == undefined){
         this.changes = [];
        }else{
          this.changes = response5.specialcontractChanges
        }
        });
