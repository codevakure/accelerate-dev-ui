...
@Component({
  selector: 'app-a-packaging',
  templateUrl: './a-packaging.component.html',
  styleUrls: ['./a-packaging.component.css']
})
export class APackagingComponent implements OnInit {
    ...
    if(event.html != undefined){
      ...
      if(event.source != 'api' &&  event.html == undefined){
        if(this.data != this.orginaldata){
          let change = `<b>PACKAGING AND MARKING</b> was updated. `,data4
          this.changes.push(change);
          data4={
            packagingChanges:this.changes
          }
  ...
  getSectiondata() {
    this.ap.getsoldappids(this.solno).subscribe(response => {
      ...
      .subscribe(response => {
       ...
       // this.data=response.description? response.description == '<div><br></div>'?'<div><h3><b>D. Packaging and Marking</b></h3></div><div><br></div>':response.description:'<div><h3><b>D. Packaging and Marking</b></h3></div><div><br></div>';
      });
    ...
    .subscribe((response5) => {
      ...
      console.warn(typeof response5.packagingChanges)
      if( response5.packagingChanges == undefined){
       this.changes = [];
      }else{
        this.changes = response5.packagingChanges
      }
